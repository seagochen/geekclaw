...
func TestSplitMessage(t *testing.T) {
	...
	tests := []struct {
		...
		content      string
		maxLen       int
		expectChunks int                                 // 检查分块数量
		checkContent func(t *testing.T, chunks []string) // 自定义验证
	}{
		{
			name:         "Empty message",
			content:      "",
		...
		},
		{
			...
			// 1750 个字符后跟换行符，再跟更多字符。
			// 动态缓冲区：2000 / 10 = 200。
			// 有效限制：2000 - 200 = 1800。
			// 由于换行符在位置 1750（< 1800），分割应发生在换行符处。
			// 总长度必须 > 2000 才会触发分割。1750 + 1 + 300 = 2051。
		...
		},
		{
			...
			content:      strings.Repeat("\u4e16", 2500), // 2500 个 rune，7500 字节
			maxLen:       2000,
			expectChunks: 2,
			checkContent: func(t *testing.T, chunks []string) {
				// 验证分块包含有效的 Unicode 字符且不会从 rune 中间分割
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(got) != tc.expectChunks {
				...
				return // 如果分块数量假设特定的分割方式，则停止进一步检查
			}
...
func TestFindLastNewlineInRange(t *testing.T) {
	...
	tests := []struct {
		...
		{"narrow window misses newline outside window", 4, 11, 3, 3}, // 返回 start-1（未找到）
