...
func TestNewJSONLStore_CreatesDirectory(t *testing.T) {
	...
	metaDir := filepath.Join(base, "nested", "meta")
	store, err := NewJSONLStore(dir, metaDir)
	if err != nil {
		t.Fatalf("NewJSONLStore: %v", err)
	}
	defer store.Close()

	for _, d := range []string{dir, metaDir} {
		info, err := os.Stat(d)
		if err != nil {
			t.Fatalf("Stat(%s): %v", d, err)
		}
		if !info.IsDir() {
			t.Errorf("expected directory for %s, got file", d)
		}
	}
}
...
func TestPersistence_AcrossInstances(t *testing.T) {
	...
	store1, err := NewJSONLStore(dir, metaDir)
	if err != nil {
		t.Fatalf("NewJSONLStore: %v", err)
	}
	...
	store2, err := NewJSONLStore(dir, metaDir)
	if err != nil {
		t.Fatalf("NewJSONLStore: %v", err)
	}
	defer store2.Close()
...
func BenchmarkAddMessage(b *testing.B) {
	base := b.TempDir()
	store, err := NewJSONLStore(filepath.Join(base, "s"), filepath.Join(base, "m"))
	if err != nil {
