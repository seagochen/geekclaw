...
func TestAgentModelConfig_UnmarshalString(t *testing.T) {
	...
	if err := yaml.Unmarshal([]byte(`gpt-4`), &m); err != nil {
...
func TestAgentModelConfig_MarshalString(t *testing.T) {
	...
	if strings.TrimSpace(string(data)) != "gpt-4" {
...
// TestDefaultConfig_PluginsDir verifies plugins dir path is correctly set
func TestDefaultConfig_PluginsDir(t *testing.T) {
	cfg := DefaultConfig()

	if cfg.Agents.Defaults.PluginsDir == "" {
...
// TestDefaultConfig_Channels verifies no channels are configured by default
func TestDefaultConfig_Channels(t *testing.T) {
	cfg := DefaultConfig()

	if len(cfg.Channels.External) != 0 {
		t.Error("No external channels should be configured by default")
	}
}
...
func TestDefaultConfig_WebTools(t *testing.T) {
	...
	// Web tool should be enabled by default
	if !cfg.Tools.Web.Enabled {
		t.Error("Expected web tools to be enabled by default")
	}
	// Plugins map should be nil (no built-in providers)
	if len(cfg.Tools.Web.Plugins) != 0 {
		t.Error("Expected no web search plugins by default")
	}
}
...
func TestDefaultConfig_PluginsDir_Default(t *testing.T) {
	// Unset to ensure we test the default
	t.Setenv("GEEKCLAW_HOME", "")
	// Set a known home for consistent test results
	t.Setenv("HOME", "/tmp/home")

	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".geekclaw", "plugins")

	if cfg.Agents.Defaults.PluginsDir != want {
		t.Errorf("Default plugins dir path = %q, want %q", cfg.Agents.Defaults.PluginsDir, want)
	}
}

func TestAgentDefaults_WorkspaceAndPluginsDir(t *testing.T) {
	// workspace and plugins_dir are separate fields.
	yamlData := "workspace: /my/workspace\nplugins_dir: /my/plugins\nrestrict_to_workspace: true\nmax_tokens: 1024\n"
	var d AgentDefaults
	if err := yaml.Unmarshal([]byte(yamlData), &d); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if d.Workspace != "/my/workspace" {
		t.Errorf("Workspace = %q, want %q", d.Workspace, "/my/workspace")
	}
	if d.PluginsDir != "/my/plugins" {
		t.Errorf("PluginsDir = %q, want %q", d.PluginsDir, "/my/plugins")
	}
...
func TestAgentDefaults_RestrictToPluginsDirMigration(t *testing.T) {
	// Old field restrict_to_plugins_dir should migrate to RestrictToWorkspace.
	yamlData := "workspace: /ws\nplugins_dir: /dir\nrestrict_to_plugins_dir: true\n"
	var d AgentDefaults
	if err := yaml.Unmarshal([]byte(yamlData), &d); err != nil {
		t.Fatalf("unmarshal: %v", err)
	}
	if !d.RestrictToWorkspace {
		t.Error("RestrictToWorkspace should be true (migrated from restrict_to_plugins_dir)")
	}
}

func TestDefaultConfig_PluginsDir_WithGeekclawHome(t *testing.T) {
	t.Setenv("GEEKCLAW_HOME", "/custom/geekclaw/home")

	cfg := DefaultConfig()
	want := "/custom/geekclaw/home/plugins"

	if cfg.Agents.Defaults.PluginsDir != want {
		t.Errorf("PluginsDir path with GEEKCLAW_HOME = %q, want %q", cfg.Agents.Defaults.PluginsDir, want)
	}
}
