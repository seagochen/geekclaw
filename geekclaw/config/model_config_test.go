...
func TestAgentDefaults_YAML_BackwardCompat(t *testing.T) {
	tests := []struct {
		name     string
		yaml     string
		wantName string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := yaml.Unmarshal([]byte(tt.yaml), &defaults); err != nil {
...
func TestFullConfig_YAML_BackwardCompat(t *testing.T) {
